...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const profileUrl = `https://tapin.live/${username}`;
  const images = avatarUrl 
    ? [{ url: avatarUrl, width: 400, height: 400, alt: `${user.name}'s profile picture` }]
    : [{ url: "/og.png", width: 1200, height: 630, alt: "TapIn" }];

  return {
    title: `${user.name} | TapIn`,
    description: user.bio || `Visit ${user.name}'s profile on TapIn`,
    metadataBase: new URL("https://tapin.live"),
    openGraph: {
      title: `${user.name} | TapIn`,
      description: user.bio || `Visit ${user.name}'s profile on TapIn`,
      url: profileUrl,
